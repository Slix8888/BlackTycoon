...
//alt.on('keyup', (key) => {
  //  if (alt.Player.local.getSyncedMeta("ADMINLEVEL") <= 6) return;
    //if (key == 121) { //F10 
      //  isNameTagVisible = !isNameTagVisible;
   // }
//});

alt.onServer('Clientq8FqUh28kGTAELbX:Nametag:names', () => {
    isNameTagVisible = !isNameTagVisible;
});

alt.onServer('Clientq8FqUh28kGTAELbX:Admin:Invincible', () => {
    game.setEntityInvincible(alt.Player.local.scriptID, true);
});

alt.onServer('Clientq8FqUh28kGTAELbX:Admin:Invincible1', () => {
