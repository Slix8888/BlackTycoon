...
function handleCamera() {
    ...
    const newx = pos.x - movementVector.x + rightVector.x;
    const newy = pos.y - movementVector.y + rightVector.y;
    const newz = pos.z - movementVector.z + rightVector.z + zSpeedUp - zSpeedDown;

    native.setCamCoord(cam, newx, newy, newz);
    native.setCamRot(cam, rot.x + rightAxisY * -5.0, 0.0, rot.z + rightAxisX * -5.0, 2);

    emitServer('freecam:Update', newx, newy, newz);

    /**if (Date.now() > nextUpdate) {
        nextUpdate = Date.now() + timeBetweenPlayerUpdates;
        emitServer('freecam:Update', newPos);
    }**/
}